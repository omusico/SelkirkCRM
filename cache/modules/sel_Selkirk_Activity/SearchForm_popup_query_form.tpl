...
<tr>
	...
	<td scope="row" nowrap="nowrap" width='10%' >
		...
		<label for='name_advanced'>{sugar_translate label='LBL_NAME' module='sel_Selkirk_Activity'}</label>
    	</td>
...
<input type='text' name='{$fields.name_advanced.name}' 
	...
	<td scope="row" nowrap="nowrap" width='10%' >
		...
		<label for='contacted_by_advanced'>{sugar_translate label='LBL_CONTACTED_BY' module='sel_Selkirk_Activity'}</label>
    	</td>
	<td  nowrap="nowrap" width='30%'>
			
<input type="text" name="{$fields.contacted_by_advanced.name}"  class="sqsEnabled"   id="{$fields.contacted_by_advanced.name}" size="" value="{$fields.contacted_by_advanced.value}" title='' autocomplete="off"  >
<input type="hidden"  id="{$fields.user_id_c_advanced.name}" value="{$fields.user_id_c_advanced.value}">
<span class="id-ff multiple">
<button type="button" name="btn_{$fields.contacted_by_advanced.name}"   title="{$APP.LBL_SELECT_BUTTON_TITLE}" class="button firstChild" value="{$APP.LBL_SELECT_BUTTON_LABEL}" onclick='open_popup("{$fields.contacted_by_advanced.module}", 600, 400, "", true, false, {literal}{"call_back_function":"set_return","form_name":"search_form","field_to_name_array":{"id":"user_id_c_advanced","name":"contacted_by_advanced"}}{/literal}, "single", true);'>{sugar_getimage alt=$app_strings.LBL_ID_FF_SELECT name="id-ff-select" ext=".png" other_attributes=''}</button><button type="button" name="btn_clr_{$fields.contacted_by_advanced.name}"   title="{$APP.LBL_CLEAR_BUTTON_TITLE}" class="button lastChild" onclick="this.form.{$fields.contacted_by_advanced.name}.value = ''; this.form.{$fields.user_id_c_advanced.name}.value = '';" value="{$APP.LBL_CLEAR_BUTTON_LABEL}">{sugar_getimage name="id-ff-clear" alt=$app_strings.LBL_ID_FF_CLEAR ext=".png" other_attributes=''}</button>
</span>
